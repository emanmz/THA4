...
for k = 1:n_steps
    ...
    %% Primary: TOOL-TIP IK (from lecture slides)

    J = geometric_jacobian(q, DH, d_flange, d_tool);

    Jv = J(1:3,:);     % linear velocity Jacobian
    Jw = J(4:6,:);     % angular velocity Jacobian

    % Tool offset in EE frame (z-axis direction)
    t_local = [0; 0; d_tool];
    ...
    % Tool Jacobian (the slide formula) from w14-l1
...
%% FIGURE 4A: Joint Angle Histories (Part A)
figure('Color','w','Name','Joint Angles Part A','Position',[50 100 1200 500]);
...
for j = 1:7
    subplot(2,4,j);
    plot(iters, rad2deg(a.q(j,:)), 'Color',colors(1,:), 'LineWidth',1.5); hold on;
    yline(rad2deg(q_min_r(j)), 'r--', 'LineWidth',1);
    yline(rad2deg(q_max_r(j)), 'r--', 'LineWidth',1);
...
sgtitle('Part A — Joint Angle Histories with Limits (red dashed)', ...
    'FontSize',13,'FontWeight','bold');

saveas(gcf,'Fig4A_Joint_Angles_PartA.png');


%% FIGURE 4B: Joint Angle Histories (Part B)
figure('Color','w','Name','Joint Angles Part B','Position',[50 100 1200 500]);

for j = 1:7
    subplot(2,4,j);
    plot(iters, rad2deg(b.q(j,:)), 'Color',colors(2,:), 'LineWidth',1.5); hold on;
    yline(rad2deg(q_min_r(j)), 'r--', 'LineWidth',1);
    yline(rad2deg(q_max_r(j)), 'r--', 'LineWidth',1);
...
sgtitle('Part B — Joint Angle Histories with Limits (red dashed)', ...
    'FontSize',13,'FontWeight','bold');

saveas(gcf,'Fig4B_Joint_Angles_PartB.png');
