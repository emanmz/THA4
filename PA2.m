...
% Ax3: Tool Orientation (Z-axis components)
...
legend(ax3, {'Zx','Zy','Zz'}, 'TextColor', 'w', 'Location', 'southeast');

% % Ax4: Joint Angles (Black Background + Uniform Color)
% ax4 = subplot(2,2,4);
% hold(ax4, 'on');
% set(ax4, 'Color', 'k', 'XColor', 'w', 'YColor', 'w'); % Black background, white axes
% title(ax4, 'Live Joint Angles', 'Color', 'w');
% 
% % Initialize with a uniform color (e.g., [0 0.7 0.7] for teal)
% hBar = bar(ax4, rad2deg(q), 'FaceColor', [0 0.7 0.7], 'EdgeColor', 'w'); 
...
% end
% 
% % Set hBar to use 'flat' CData so we can still highlight specific bars in red
% hBar.FaceColor = 'flat';
% hBar.CData = repmat([0 0.7 0.7], 7, 1); % Initial uniform teal color

% Ax4: Joint Angles (Shadow + Active Bars)
ax4 = subplot(2,2,4);
hold(ax4, 'on');
...
hShadow.BaseValue = min(rad2deg(qL)); % This helps align the base
% Set the shadow bar property to 'stacked' or simply offset the bottom
% A cleaner way is using 'errors bars' or a rectangle, but a second bar works:
for i = 1:7
...
% --- ACTIVE BARS (Same as before) ---
uniformColor = [0 0.7 0.7]; % Teal
...
pHist = zeros(3,N);
eHist = zeros(1,N);
zHist = zeros(3,N); % To store the tool's Z-axis vector
qHist = zeros(7, N);
...
b_ori = zeros(3,1); % pure rotational objective
...
% combine objective functions for solver
A = [zeta*A_pos;
    eta*A_ori];
...
%     set(hBar, 'YData', qDeg);
%     threshold = 0.05; 
%     uniformColor = [0 0.7 0.7]; % Matches the color set above
% for i = 1:7
%     % Update Text Label Position
%     yOffset = sign(qDeg(i)) * 15; % Moves text above or below the bar
%     set(hTexts(i), 'Position', [i, qDeg(i) + yOffset, 0], 'String', sprintf('%.1f°', qDeg(i)));
% 
%     % Limit Proximity Logic
%     range = qU(i) - qL(i);
%     if (qU(i) - q(i) < range * threshold) || (q(i) - qL(i) < range * threshold)
%         hBar.CData(i,:) = [1 0 0]; % Highlight Red
%         set(hTexts(i), 'Color', 'r');
%     else
%         hBar.CData(i,:) = uniformColor; % Reset to Uniform Color
