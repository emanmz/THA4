...
p_current = toolTipPosition(M, S_space, q, toolL);
...
function p_tip = toolTipPosition(M, S_space, q, toolL)
% Computes tool tip position in space frame using PoE FK

% Forward kinematics (flange frame)
T = FK_space_no_plot(M, S_space, q);

% Tool tip in local tool frame (along +Z)
p_local = [0; 0; toolL];

% Transform to space frame
p_tip = T(1:3,1:3) * p_local + T(1:3,4);
end